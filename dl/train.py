        ...
        with torch.no_grad():
            for i_batch, batched_graph in enumerate(dl, start=1):
                ...
                if self.mis_aly:
                    ...
                    sample = batched_graph.ndata['org_feat'].to(torch.device("cpu")).numpy()
                    df = pd.DataFrame(sample, columns = ['sim','cluster_id', 'state', 'node'])
                    df['label'] = y_truelabel
                    score = y_pred.to(torch.device("cpu")).numpy()
                    df['pred_label'] = y_predlabel
                    df['pred_score_0'] = score[:,0].flatten()
                    df['pred_score_1'] = score[:,1].flatten()
                    df['pred_score_2'] = score[:,2].flatten()
                    if i_batch == 1:
                        header_flag = True
                    else:
                        header_flag = False
                    ...
                    df.to_csv(f'../test/mis_aly/mis_aly_{self.args.model_num}.csv', mode='a', index=False, header=header_flag)
